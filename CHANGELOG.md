...
- [BREAKING] Removed private note support. `/get_tokens` now only creates public notes: the `is_private_note` query parameter was removed, and the `/get_note` and `/send_note` endpoints were removed along with the `--note-transport-url` flag, the `MIDEN_FAUCET_NOTE_TRANSPORT_URL` env var and the `note_transport_url` field of `/get_metadata`.
- [BREAKING] `start` no longer loads a faucet account, so it runs without `init`. The token's decimals come from the new required `--decimals` flag, the frontend shows the funding service's account as the faucet address, and the `/issuance` endpoint, the `max_supply` field of `/get_metadata` and the `--batch-size` flag were removed along with the issuance bar.
- [BREAKING] Token requests are forwarded to a funding service instead of being minted. `/get_tokens` calls the service, which creates a public P2ID note and answers once it is committed, so `start` now requires `--funding-service-url` and refuses to run if the service is unreachable or if `--max-claimable-amount` exceeds the service's own maximum.
- [BREAKING] `init` can no longer create a faucet account and requires `--import` with `--faucet-account-id`, so `--token-symbol`, `--decimals` and `--max-supply` were removed along with their env vars. Creating one needs the chain's fee asset, which 0.17 moved into the protocol config, and the client cannot read it.
- Updated the Miden dependencies to the 0.17 protocol: `miden-client` / `miden-client-cli` / `miden-client-sqlite-store` / `miden-node-proto-build` to v0.17.0-rc.1, `miden-testing` to v0.17.0-rc.5, and the frontend SDK and wallet adapter to v0.17.0-rc.1.
- [BREAKING] Removed fee sponsorship and operator funding. The faucet no longer attaches `FEE_SPONSORSHIP` notes to its MINT notes, nor mints P2ID notes to top up the operator account, and the note screener no longer tracks them. This clears the way for the 0.17 protocol, where the chain's fee asset moved out of the block header and into the protocol config.
